  ...
  if (IS_ANDROID) {
    ...
    async function send() {
      ...
      function getLocalTime() {
        ...
        // put this near the top of your IIFE, before buildWidget()
        const SESSION_ID = (() => {
          // reuse if the widget re-initializes on the same page
          if (typeof window !== 'undefined' && window.__OAT_SESSION_ID) return window.__OAT_SESSION_ID;

          // make a hyphen-free UUID (fallback if crypto unsupported)
          let id;
          try {
          ...
          if (typeof window !== 'undefined') window.__OAT_SESSION_ID = id;
          return id;
        })();
      ...
      try {
        ...
        const payload = { message: text, history: history.slice(-8), id: id, time: getLocalTime(), session_id: SESSION_ID};
    ...
    //   setTimeout(async () => {
    //     try {
    //       console.log("Full date:", getLocalTime());
    //       const payload = { message: text, history: history.slice(-8), id, time: getLocalTime()};
    //       const data = await callApi(endpoint, payload);
    //       const n = normalize(data);
